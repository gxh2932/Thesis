...
def outer_product(v, w):
    """
    compute the outer product of two lists of variables xs,ys
    :param xs:
    :param ys:
    :return:
    """
    return [torch.sum(x * y) for (x, y) in zip(v, w)]
...
class hessian():
    ...
    def diag(self, maxIter=100, tol=1e-3):
        """
        compute the diagonal of hessian using Hutchinson's method
        maxIter: maximum iterations used to compute diagonal
        tol: the relative tolerance
        """
        ...
        diag_vhv = [
            torch.zeros(p.size(), device=device)
            for p in self.params
        ]
        ...
        for i in range(maxIter):
            ...
            diag_vhv += outer_product(v, Hv)

        diag = [
            diag_vhv_i  / maxIter for diag_vhv_i in diag_vhv
        ]

        diag_clean = [
            diag_i.cpu().detach().numpy().item() if len(diag_i.size()) == 0 else 0
            for diag_i in diag
        ]
