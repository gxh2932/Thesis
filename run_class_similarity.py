...
def similarity_stuff(model_directory, trained_model_directory):
    d_path = "./"
        ...
        if file.endswith(end_prefix):
            try:
                model.load_state_dict(torch.load(os.path.join('trained', model_directory, trained_model_directory,
                                                              file)))
                similarity = utils.compute_similarity(model, trainloader, n_classes)
                with open(os.path.join('trained', model_directory, trained_model_directory, 'class_similarity.txt'),
                          'w') as f:
...
if __name__ == '__main__':
    ...
    similarity_stuff(model_dir, trained_model_dir)